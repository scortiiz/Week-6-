...
# # # In the indexing you just did (playlist[1:4]), if you remove the first index (1), what songs do you think will be in the output? When you omit the first index like that, Python automatically starts your list at the beginning of the list: Im that girl, alien superstar, cuff it, energy
# print(playlist[:4])

# # # Similarly, if you remove the second index (4), what songs do you think will be in the output? When you omit the second index, Python automatically starts counting from the first index and includes the rest of the list. So, if your first index is 2, that means that your output will start at the element at location 2 (3rd element) and then print out the rest of the list from that location: Starts at cuff it then the rest of the songs
# print(playlist[2:])

# # # How about if your index has a negative number? That just means that you are counting from the back of the list. That means if you put -3, the last three songs from the list will be printed in the output: All songs after All up in Your Mind
# print(playlist[-3:])
